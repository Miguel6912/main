...
- [x] **1.2 Enemy defense + player crit** — `defense` (0-3) added per enemy type in `ENEMY_TYPES`/`createEnemy`, threaded through `resolveDamage` in `applyDamageToEnemy`. Flat 12% crit chance / 1.75x multiplier on every player hit (melee or ranged) for now -- deliberately generic; 1.3's weapon hooks are what will push these numbers around per weapon. Verified via headless simulation: exact defense-value assertions, damage-reduction averaged over 5000 rolls, crit rate measured at 12.8% over 400 hits against a 12% target.
- [x] **1.3/1.4 Weapon identity, all five** — done together rather than split, since the whole point is the system, not a proof-of-concept on two. Each weapon's `ability` (`items.js` `WEAPON_CATALOG`) is data consulted generically by `applyDamageToEnemy`/`performPlayerAttack` (`game.js`), not per-weapon branches:
  - **Dagger** — `postDodgeCritBonus`: +40% crit chance for `POST_DODGE_WINDOW` (0.5s) after a dodge ends.
  - **Sword** — `riposteWindupBonus`/`riposteInvuln`: 1.5x damage + 0.3s invuln for hitting an enemy still mid-windup.
  - **Axe** — `defenseIgnore`/`executeThreshold`/`executeBonus`: ignores 60% of enemy defense; 1.8x damage finishing anything below 25% hp.
  - **Greatsword** — `knockbackMult`/`hitstunMult`/`waveDamageMult`: 1.8x knockback, 1.6x hitstun, and its own combo-finisher wave hits harder on top of the baseline multiplier every weapon gets.
  - **Bow** — `chargeable`: hold the attack input (new `attackHeld` in the input object) to charge up to +100% damage over `maxChargeTime` (0.9s); a quick tap still fires near base damage. New gold ring telegraph while charging, bigger/glowing arrow sprite on a near-full-charge shot.
  All five verified headlessly (exact damage/knockback/hitstun/invuln/crit-rate assertions, thousands of samples where statistical). Caught and fixed two real bugs in the process: a same-frame tap-and-release that never registered as "held" could silently fire nothing at all (bow), and firing a bow crashed the entire render loop on the next frame (`pickPlayerClipName` looked up a nonexistent `swing-1` animation clip for a weapon that doesn't combo) -- a genuine pre-existing bug the headless smoke test could never have caught, since it never calls `renderGame` at all.
