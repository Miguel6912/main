...
- [ ] **1.7 Prove it out on one real boss** — wire *one* of the 8 already-painted boss sheets (`ashwing`, `bellkeeper`, `clover`, `gullet_king`, `hollow_regent`, `mirror_scholar`, `thorn_hart`, `vesper`) through 1.6 end-to-end. **Open decision**: none of those 8 names obviously *is* the Hollow Hare from `story.js` — we should pick together whether one becomes the Hare, one becomes a fresh rumour-boss, or the Hare waits for its own art.
